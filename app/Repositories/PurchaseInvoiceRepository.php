    ...
    public function storeSalesItems($input, $owner)
    {
        ...
        foreach ($input['itemsArr'] as $record) {
            ...
            if (isset($record['service_id'])) {
                $purchaseItem = PurchaseItem::find($record['service_id']);
            }
            ...
            $data['product_unit_id'] = $record['unit_id'] ?? null;
            $data['item'] = $record['item'] ?? ' ';
            $data['description'] = $record['description'] ?? ' ';
            $data['rate'] = $record['rate'] ?? 0;
            $data['tax'] = $record['tax'] ?? 0;
            ...
            if ($purchaseItem && isset($record['quantity'])) {
                ...
                if ($owner->getOwnerType() === 'App\Models\PurchaseInvoice') {
                    // Purchase Invoice: Add to stock (new inventory)
                    $purchaseItem->qty_in += $quantity;
                    $purchaseItem->qty_current += $quantity;
                    $purchaseItem->stock = $purchaseItem->qty_current;
                }
                // Check if this is a purchase return
                else if ($owner->getOwnerType() === 'App\Models\PurchaseReturn') {
                    // Purchase Return: Subtract from stock (return inventory)
                    // First check if we have enough current stock to return
                    if ($purchaseItem->qty_current < $quantity) {
                        // If not enough stock, adjust quantity to available stock
                        $quantity = $purchaseItem->qty_current;
                    }

                    $purchaseItem->qty_current -= $quantity;
                    $purchaseItem->stock = $purchaseItem->qty_current;

                    // Track returned quantity if you have that field
                    if (isset($purchaseItem->qty_returned)) {
                        $purchaseItem->qty_returned += $quantity;
                    }
                }
                ...
                // Ensure qty_current doesn't go below zero
                if ($purchaseItem->qty_current < 0) {
                    $purchaseItem->qty_current = 0;
                }
