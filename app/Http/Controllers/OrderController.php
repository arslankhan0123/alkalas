...
class OrderController extends AppBaseController
{
    ...
    public function create($customerId = null)
    {
        ...
        $units = $this->purchaseItemRepository->getUnits();
        $paymentModes = $this->purchaseItemRepository->getPaymentModes();
        $usersBranches = $this->purchaseItemRepository->getUsersBranches();
        $branches = Branch::pluck('name', 'id')->toArray();
        // Fetch purchase items for dropdown (similar to quotation)
    ...
    public function show(PurchaseOrder $order)
    {
        ...
        foreach ($estimate->salesItems as $item) {
            ...
            $rowSubtotal = $item->quantity * $item->rate;

            // Apply item-level discount (percentage)
            $itemDiscount = $rowSubtotal * ($item->discount / 100);
            $itemTaxable = $rowSubtotal - $itemDiscount;

            // Calculate VAT for this item
            $itemVATAmount = $itemTaxable * ($item->tax / 100);
            $itemTotal = $itemTaxable + $itemVATAmount;

            // Accumulate totals
            $totalExcludingVAT += $rowSubtotal;
            $totalItemDiscount += $itemDiscount;
            $totalTaxableAmount += $itemTaxable;
            $totalVATAmount += $itemVATAmount;
            $totalIncludingVAT += $itemTotal;
        }

        // Handle invoice-level discount
        $invoiceDiscount = $estimate->discount ?? 0;
        $invoiceDiscountAmount = 0;

        if (isset($estimate->discount_type) && $estimate->discount_type == 0) {
            // Percentage discount on total taxable amount
            $invoiceDiscountAmount = ($invoiceDiscount / 100) * $totalTaxableAmount;
        } else {
        ...
        $afterInvoiceDiscount = $totalTaxableAmount - $invoiceDiscountAmount;
        $finalVAT = $afterInvoiceDiscount * 0.05; // Apply VAT on discounted taxable amount
        $grandTotal = $afterInvoiceDiscount + $finalVAT;

        $words = $this->amountToWords($grandTotal);

        $status = Task::STATUS;
        $priorities = Task::PRIORITY;
        $groupName = (request('group') == null) ? 'estimate_details' : (request('group'));

        return view("purchase-orders.views.$groupName", compact(
            'estimate',
            'status',
            'priorities',
            'groupName',
    ...
    public function edit(PurchaseOrder $order)
    {
        ...
        $usersBranches = $this->purchaseItemRepository->getUsersBranches();
        $branches = Branch::pluck('name', 'id')->toArray();
        // Get purchase items for dropdown
        $purchaseItems = PurchaseItem::select('id', 'code', 'name', 'price', 'stock', 'unit_id')
            ->with('productUnit:id,title')
            ->get()
            ->map(function ($item) {
