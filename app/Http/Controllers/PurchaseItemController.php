...
class PurchaseItemController extends AppBaseController
{
    ...
    public function create()
    {
        ...
        $settingItemCode = $this->purchaseItemRepository->getSettingSyncList();
        $maxCode = PurchaseItem::max('code');
        if((int)$settingItemCode['item_no'] > (int)$maxCode){
            $nextNumber = (int)$settingItemCode['item_no'];
        }
        elseif((int)$settingItemCode['item_no'] == (int)$maxCode){
              $nextNumber = (int)$settingItemCode['item_no'] +1;
        }
        else{
            $nextNumber = $maxCode ? ((int) $maxCode + 1) : 1;
        }
        $purchaseCodeLatest = str_pad($nextNumber, 2, '0', STR_PAD_LEFT);
        return view('purchase_items.create', compact(['groups', 'categories', 'subcategories', 'units', 'nextNumber', 'brands', 'sizes', 'colors', 'itemCodeStatus','purchaseCodeLatest']));
    }
