...
class BranchController extends AppBaseController
{
    ...
    public function create()
    {
        ...
        $states = $this->branchRepository->getStates();
        $cities = $this->branchRepository->getCities();
        $areas = $this->branchRepository->getAreas();
        $invoiceFormats = Branch::INVOICE_FORMATS;
        return view('branches.create', compact(['countries', 'currencies', 'company', 'banks', 'invoiceFormats','states','cities','areas']));
    }
    ...
    public function edit(Branch $branch)
    {
        ...
        return view('branches.edit', compact(['branch', 'countries', 'currencies', 'company', 'banks', 'printFormats', 'invoiceFormats','states','cities','areas']));
    }
    public function view(Branch $branch)
    {
        $branch->load('bank','country','stateData','cityData','areaData','currency');
        // return $branch;
        $printFormats = Branch::PRINT_FORMATS;
        $invoiceFormats = Branch::INVOICE_FORMATS;
